...
vertice vertice_to_point(vertice vertice1)
{
  vertice point1;
  point1.x = vertice1.x * 10000 / vertice1.z;  point1.y = vertice1.y * 10000 / vertice1.z;
  point1.z = vertice1.z;
  return point1;
}
...
{
    vertice point1 = vertice_to_point(polygon.vertice1);
    vertice point2 = vertice_to_point(polygon.vertice2);
    vertice point3 = vertice_to_point(polygon.vertice3);
    ...
    gfx_3d_fill_triangle(point1, point2, point3, polygon.color);
}
...
#define polyCount 24
obj3D_Box_colored_edge::obj3D_Box_colored_edge()
{
    ID = 0;
    reset();
}
void obj3D_Box_colored_edge::reset()
{
    hide();
    center.x = 0;
    center.y = 0;
...
void obj3D_Box_colored_edge::hide()
{
    if(ID != 0)
    {
        int i = 0;
...
void obj3D_Box_colored_edge::show()
{
    if(ID == 0)
    {
        ID = polygon_buffer_next_ID;
...
void obj3D_Box_colored_edge::rotate(vector rotate)
{
    rotation.x += rotate.x;
    rotation.y += rotate.y;
    rotation.z += rotate.z;
}
void obj3D_Box_colored_edge::move(vector move)
{
    center.x += move.x;
    center.y += move.y;
    center.z += move.z;
}
void obj3D_Box_colored_edge::scale(vector factor)
{
    dimension.x *= factor.x;
    dimension.y *= factor.y;
    dimension.z *= factor.z;
}
void obj3D_Box_colored_edge::resize(vector newsize)
{
    dimension.x = newsize.x;
    dimension.y = newsize.y;
    dimension.z = newsize.z;
}
void obj3D_Box_colored_edge::color(uint8_t newcolor)
{
    for(int i = 0; i < 6; i++)
    {
        color_arr[i] = newcolor;
...
void obj3D_Box_colored_edge::color(uint8_t newcolor[6])
{
    for(int i = 0; i < 6; i++)
    {
        color_arr[i] = newcolor[i];
...
void obj3D_Box_colored_edge::color_edge(uint8_t newcolor)
{
    for(int i = 0; i < 6; i++)
    {
        edge_color_arr[i] = newcolor;
    }
}
void obj3D_Box_colored_edge::color_edge(uint8_t newcolor[6])
{
    for(int i = 0; i < 6; i++)
    {
        edge_color_arr[i] = newcolor[i];
    }
}
void obj3D_Box_colored_edge::update()
{
    if(ID != 0)
    {
        int i = 0;
        ...
        polygon_buffer[i].vertice3 = vertices[1];
        polygon_buffer[i].color = edge_color_arr[0];
        i++;
        polygon_buffer[i].vertice1 = vertices[1];
        polygon_buffer[i].vertice2 = vertices[2];
        polygon_buffer[i].vertice3 = vertices[2];
        ...
        i++;
        polygon_buffer[i].vertice1 = vertices[2];
        polygon_buffer[i].vertice2 = vertices[3];
        polygon_buffer[i].vertice3 = vertices[3];
        polygon_buffer[i].color = edge_color_arr[1];
        i++;
        polygon_buffer[i].vertice1 = vertices[3];
        polygon_buffer[i].vertice2 = vertices[0];
        polygon_buffer[i].vertice3 = vertices[0];
        polygon_buffer[i].color = edge_color_arr[1];
        i++;
        polygon_buffer[i].vertice1 = vertices[4];
        polygon_buffer[i].vertice2 = vertices[5];
        polygon_buffer[i].vertice3 = vertices[5];
        polygon_buffer[i].color = edge_color_arr[2];
        i++;
        polygon_buffer[i].vertice1 = vertices[5];
        polygon_buffer[i].vertice2 = vertices[6];
        polygon_buffer[i].vertice3 = vertices[6];
        polygon_buffer[i].color = edge_color_arr[2];
        i++;
        polygon_buffer[i].vertice1 = vertices[6];
        polygon_buffer[i].vertice2 = vertices[7];
        polygon_buffer[i].vertice3 = vertices[7];
        polygon_buffer[i].color = edge_color_arr[3];
        i++;
        polygon_buffer[i].vertice1 = vertices[7];
        polygon_buffer[i].vertice2 = vertices[4];
        polygon_buffer[i].vertice3 = vertices[4];
        polygon_buffer[i].color = edge_color_arr[3];
        i++;
        ...
        polygon_buffer[i].vertice2 = vertices[4];
        polygon_buffer[i].vertice3 = vertices[4];
        polygon_buffer[i].color = edge_color_arr[4];
        i++;
        polygon_buffer[i].vertice1 = vertices[2];
        polygon_buffer[i].vertice2 = vertices[5];
        polygon_buffer[i].vertice3 = vertices[5];
        ...
        polygon_buffer[i].vertice2 = vertices[6];
        polygon_buffer[i].vertice3 = vertices[6];
        polygon_buffer[i].color = edge_color_arr[5];
        i++;
        polygon_buffer[i].vertice1 = vertices[3];
        polygon_buffer[i].vertice2 = vertices[7];
        polygon_buffer[i].vertice3 = vertices[7];
...
void Camera::render(uint8_t *output)
{
    ...
    {   
        ...
        float x = activepolygon.vertice1.x * op1 + activepolygon.vertice1.y * op2 + activepolygon.vertice1.z * op3;
        float y = activepolygon.vertice1.x * op4 + activepolygon.vertice1.y * op5 + activepolygon.vertice1.z * op6;
        float z = activepolygon.vertice1.x * op7 + activepolygon.vertice1.y * op8 + activepolygon.vertice1.z * op9;
        activepolygon.vertice1.x = x - center.x;
        activepolygon.vertice1.y = y - center.y;
        activepolygon.vertice1.z = z - center.z;

        x = activepolygon.vertice2.x * op1 + activepolygon.vertice2.y * op2 + activepolygon.vertice2.z * op3;
        y = activepolygon.vertice2.x * op4 + activepolygon.vertice2.y * op5 + activepolygon.vertice2.z * op6;
        z = activepolygon.vertice2.x * op7 + activepolygon.vertice2.y * op8 + activepolygon.vertice2.z * op9;
        activepolygon.vertice2.x = x - center.x;
        activepolygon.vertice2.y = y - center.y;
        activepolygon.vertice2.z = z - center.z;

        x = activepolygon.vertice3.x * op1 + activepolygon.vertice3.y * op2 + activepolygon.vertice3.z * op3;
        y = activepolygon.vertice3.x * op4 + activepolygon.vertice3.y * op5 + activepolygon.vertice3.z * op6;
        z = activepolygon.vertice3.x * op7 + activepolygon.vertice3.y * op8 + activepolygon.vertice3.z * op9;
        activepolygon.vertice3.x = x - center.x;
        activepolygon.vertice3.y = y - center.y;
        activepolygon.vertice3.z = z - center.z;
